...
def load_data(messages_filepath, categories_filepath):
    '''
    i/p :   messages_filepath   :   Message dataset absolute path
            categories_filepath :   Categories dataset absolute path
            
    o/p :   df                  :   Combined dataset
    '''
...
def clean_data(df):
    '''
    i/p :   df                  :   Comnined dataset
            
    o/p :   df                  :   Cleaned Dataset   
    '''
    categories = df.categories.str.split(';', expand=True)
    categories.columns = categories.loc[0].apply(lambda x: x[:-2]).values.tolist()
